...
class TestProjectCreation(TestCase):
    ...
    def __init__(self):
        super().__init__(
            "Project Creation", 
            "Verify that an agent can create a project and that the agent's state and simulation are updated correctly"
        )
    ...
    async def run(self) -> bool:
        ...
        # Create simulation
        simulation = Simulation(agents=[agent], vector_store_manager=vector_store, scenario=SCENARIO)
        ...
        state_correct = (
            agent_state.current_project_id == project_id and
            agent_state.current_project_affiliation == project_name
        )
        
        # Check if the project was added to simulation
        project_added = (
            project_id in simulation.projects and
            simulation.projects[project_id]["name"] == project_name and
            simulation.projects[project_id]["description"] == project_description and
            agent.agent_id in simulation.projects[project_id]["members"]
        )
...
class TestProjectJoining(TestCase):
    ...
    def __init__(self):
        super().__init__(
            "Project Joining", 
            "Verify that an agent can join an existing project with proper state updates and resource deductions"
        )
    ...
    async def run(self) -> bool:
        ...
        creator = Agent(
            ...
            initial_state={
                "name": "ProjectCreator",
                "current_role": "Innovator",
                "goals": [{"description": "Create a project for others to join", "priority": "high"}],
                "influence_points": 30,
                "data_units": 30
            }
        )
        ...
        simulation = Simulation(agents=[creator, joiner], vector_store_manager=vector_store, scenario=SCENARIO)
        ...
        state_correct = (
            joiner_state.current_project_id == project_id and
            joiner_state.current_project_affiliation == project_name
        )
...
class TestProjectMembershipLimit(TestCase):
    ...
    def __init__(self):
        super().__init__(
            "Project Membership Limit", 
            f"Verify that project membership is limited to MAX_PROJECT_MEMBERS ({config.MAX_PROJECT_MEMBERS})"
        )
        ...
        try:
            ...
            creator = Agent(
                ...
                initial_state={
                    "name": "LimitCreator",
                    "current_role": "Innovator",
                    "goals": [{"description": "Create a project with limited spots", "priority": "high"}],
                    "influence_points": 30,
                    "data_units": 30
                }
            )
            ...
            # Create simulation
            simulation = Simulation(agents=agents, vector_store_manager=vector_store, scenario=SCENARIO)
            ...
            for i in range(1, config.MAX_PROJECT_MEMBERS):
                ...
                if not join_success:
                    self.passed = False
                    self.result_message = f"Joiner {joiner.agent_id} failed to join, but should have succeeded"
                    return False
            ...
            resources_preserved = (
                failed_joiner.state.ip == failed_initial_ip and
                failed_joiner.state.du == failed_initial_du
            )
            
            # Check if the failed joiner's state wasn't updated
            state_preserved = (
                failed_joiner.state.current_project_id is None and
                failed_joiner.state.current_project_affiliation is None
            )
            
            # Check if the failed joiner wasn't added to project
            not_in_members = failed_joiner.agent_id not in simulation.projects[project_id]["members"]
            
            # Check if the project has exactly MAX_PROJECT_MEMBERS members
            correct_member_count = len(simulation.projects[project_id]["members"]) == config.MAX_PROJECT_MEMBERS
            
            self.passed = (
                limit_enforced and 
                resources_preserved and 
                state_preserved and 
                not_in_members and 
...
class TestProjectLeaving(TestCase):
    ...
    def __init__(self):
        super().__init__(
            "Project Leaving", 
            "Verify that an agent can leave a project and that the agent's state and simulation are updated correctly"
        )
    ...
    async def run(self) -> bool:
        ...
        creator = Agent(
            ...
            initial_state={
                "name": "LeaveCreator",
                "current_role": "Innovator",
                "goals": [{"description": "Create a project for testing leaving", "priority": "high"}],
                "influence_points": 30,
                "data_units": 30
            }
        )
        ...
        # Create simulation with both agents
        simulation = Simulation(agents=[creator, leaver], vector_store_manager=vector_store, scenario=SCENARIO)
        ...
        state_correct = (
            leaver_state.current_project_id is None and
            leaver_state.current_project_affiliation is None
        )
...
class TestProjectEdgeCases(TestCase):
    ...
    def __init__(self):
        super().__init__(
            "Project Edge Cases", 
            "Verify edge cases like joining non-existent projects and leaving projects the agent isn't in"
        )
    ...
    async def run(self) -> bool:
        ...
        resources_preserved_after_join = (
            agent.state.ip == initial_ip and
            agent.state.du == initial_du
        )
        state_preserved_after_join = (
            agent.state.current_project_id is None and
            agent.state.current_project_affiliation is None
        )
        ...
        first_project_id = simulation.create_project(project_name, agent.agent_id, "First project with this name")
        ...
        duplicate_project_id = simulation.create_project(project_name, agent.agent_id, "Second project with this name")
        ...
        other_project_id = simulation.create_project(other_project_name, agent.agent_id, "Project to test leaving when not a member")
        ...
        self.passed = (
            join_failed and 
            resources_preserved_after_join and 
            state_preserved_after_join and
            leave_failed and 
            resources_preserved_after_leave and 
            state_preserved_after_leave and
            duplicate_creation_failed and
            second_leave_failed
        )
...
async def run_tests():
    ...
    for test_case in test_cases:
        try:
            ...
            print(f"{RED}❌ TEST ERROR: {test_case.name} - {str(e)}{RESET}")
            results.append(f"Test Case: {test_case.name} - ERROR\n{test_case.description}\nError: {str(e)}\n")
            failed_tests += 1
