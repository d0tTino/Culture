...
4. **L3 Summaries**: Long-term "arc" summaries synthesized from multiple L2 chapters
...
6. **Arc Summaries**: L2 chapters are occasionally consolidated into L3 arc summaries
7. **Pruning**: Lower-utility memories are removed based on age and MUS values

## Hierarchical Memory

The agent memory system is organized hierarchically into three levels:
...
- **L3 Summaries (Arc Level):** Consolidate several L2 chapters into a long-term arc summary using ``Level3SummaryManager``.
...
4. **L3 Summaries (Arcs)**: Consolidations of several chapters capturing very long-term trends
   - Generated by ``Level3SummaryManager``
   - Provide coarse context for agent history
