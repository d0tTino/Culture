...
def parse_args():
    ...
    parser.add_argument('--csv', type=str, default='request_to_operator/rag_assessment_scores.csv',
    ...
    with open(args.csv, 'r', newline='') as file:
        ...
        for row in reader:
            ...
            if not all(row.get(field, '').strip() for field in 
                      ['Relevance (1-5)', 'Coherence (1-5)', 'Completeness (1-5)', 'Accuracy (1-5)']):
                continue
            ...
            try:
                ...
                # Check score ranges
                if not all(1 <= score <= 5 for score in [relevance, coherence, completeness, accuracy]):
        ...
        for score in scenario_score_list:
            ...
            if agent_id in data.get('agents', {}) and 'queries' in data['agents'][agent_id]:
                ...
                if 0 <= query_num < len(queries):
                    ...
                    print(f"Warning: Query number {query_num} out of range for agent {agent_id} in scenario {scenario_id}")
            else:
...
def calculate_rag_metrics(data):
    ...
    if total_count > 0:
        ...
        data['rag_assessment']['success_rate'] = success_count / total_count if total_count > 0 else 0
