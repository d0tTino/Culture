...
def parse_args():
    ...
    parser.add_argument('--results_dir', type=str, default='benchmarks/tuning_results',
                       help='Path to the tuning results directory')
    parser.add_argument('--report_file', type=str, default='benchmarks/mus_threshold_tuning_report.md',
...
def main():
    ...
    try:
        with open(comparative_data_path, 'r') as f:
    ...
    for scenario_id in scenarios:
        rag_assessment_path = os.path.join(args.results_dir, scenario_id, 'rag_assessment', 
                                          f"{scenario_id}_rag_assessment.json")
        ...
        try:
            with open(rag_assessment_path, 'r') as f:
        ...
        if scenario_id in comparative_data.get('scenarios', {}):
            ...
            memory_efficiency = comparative_data['scenarios'][scenario_id].get('memory_efficiency_score', 0.0)
    ...
    best_rag_performance_id = max(
        scenarios,
        key=lambda sid: comparative_data['scenarios'][sid].get('rag_assessment', {}).get('avg_score', 0.0)
    )
    ...
    try:
        ...
        rag_scores = [comparative_data['scenarios'][sid]['rag_assessment'].get('avg_score', 0.0) / 5.0 
        ...
        memory_scores = [comparative_data['scenarios'][sid].get('memory_efficiency_score', 0.0)
                        for sid in scenario_ids]
        overall_scores = [comparative_data['scenarios'][sid].get('overall_score', 0.0)
                         for sid in scenario_ids]
...
def update_report(report_file, comparative_data):
    ...
    for scenario_id, scenario_data in comparative_data['scenarios'].items():
        ...
        rag_section.append(f"| {scenario_id} | {avg_score:.2f} | {success_rate:.2f} | {query_count} |\n")
    ...
    # Replace the section
    report_lines = report_lines[:rag_section_index] + rag_section + report_lines[next_section_index:]
    ...
    if recommendations_section_index != -1:
        ...
        for i, scenario_id in enumerate(sorted_scenarios[:5]):
            ...
            baseline_memories = comparative_data['scenarios'].get('baseline', {}).get('memory_metrics', {}).get('total_memories', 180)
            scenario_memories = scenario_data.get('memory_metrics', {}).get('total_memories', 0)
            retention_pct = (scenario_memories / baseline_memories) * 100 if baseline_memories > 0 else 0
            
            recommendations.append(f"| {i+1} | {scenario_id} | {overall_score:.3f} | {mem_efficiency:.3f} | {rag_score:.2f} | {retention_pct:.1f}% |\n")
        ...
        report_lines = report_lines[:recommendations_section_index] + recommendations + report_lines[next_section_index:]
