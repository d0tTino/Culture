...
trainset = [
    dspy.Example(
        role_name=ex["role_name"], context=ex["context"], thought=ex["thought"]
    ).with_inputs("role_name", "context")
    for ex in role_thought_examples
]
...
def thought_relevance_metric(example, prediction, trace=None):
    judge_result = judge(
        role_name=example.role_name, context=example.context, thought=prediction.thought
    )
...
def combined_metric(example, prediction, trace=None):
    return 0.5 * role_prefix_adherence_metric(
        example, prediction
    ) + 0.5 * thought_relevance_metric(example, prediction)
...
save_path = os.path.join(
    ...
    "../src/agents/dspy_programs/compiled/optimized_role_thought_generator.json",
)
...
scores = [
    combined_metric(ex, optimized_program(role_name=ex.role_name, context=ex.context))
    for ex in trainset
]
