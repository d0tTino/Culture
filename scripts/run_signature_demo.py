#!/usr/bin/env python3
"""Run the configured scenario (defaults to ``signature_demo``) and collect artifacts."""
...
SCENARIO_SLUG = "signature_demo"
SCENARIO_TITLE = SCENARIO_SLUG.replace("_", " ").title()
RESULT_DIR = Path(f"results/{SCENARIO_SLUG}")
SCENARIO_PATH = Path(f"scenarios/{SCENARIO_SLUG}.yaml")
BUNDLE_NAME = f"{SCENARIO_SLUG}_bundle.zip"
...
async def main(
    *, seed: int | None = None, steps: int | None = None, agents: int | None = None
    ...
    """Execute the configured scenario and save outputs."""
    _clean_results_dir()
    ...
    bundle_path = RESULT_DIR / BUNDLE_NAME
    export_args = [
        "--events",
        str(event_log_path),
        "--output",
