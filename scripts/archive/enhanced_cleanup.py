    ...
    for root, dirs, _ in os.walk(base_dir):
        ...
        for dir_name in dirs:
            if (
                ...
                or dir_name.startswith("test_venv")
                or "env" in dir_name.lower()
            ):
...
def remove_items(items, item_type="directory", dry_run=True):
    ...
    logger.info(
        f"{item_type.capitalize()}s {'' if not dry_run else 'that would be '}removed: {removed}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Aggressively clean up test and unnecessary files"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--keep-venv", action="store_true", help="Don't remove virtual environment directories"
    )
    ...
    if not args.dry_run:
        ...
        if final_count > 1000:
            logger.warning(
                f"Still have {final_count} files (target: <1000). Consider additional cleanup measures."
            )
        else:
