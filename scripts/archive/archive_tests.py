...
TEST_DIRS = ["tests", "benchmarks", "experiments"]

# ChromaDB test directory patterns
CHROMADB_PATTERNS = ["chroma_db_test*", "test_chroma*", "chroma_benchmark*"]
...
def archive_chroma_dirs():
    ...
    try:
                    ...
                    for root, _, files in os.walk(dir_path):
                        for file in files:
                            ...
                            arcname = os.path.join(
                                dir_path, os.path.relpath(file_path, start=dir_path)
                            )
                            zipf.write(file_path, arcname)
                            dir_files += 1
        ...
        logger.info(
            f"Successfully archived {total_files} files from {len(chroma_dirs)} ChromaDB directories to {archive_path}"
        )
...
def main():
    ...
    logger.info(
        f"Archive process complete. Archived {total_files} files from {total_archived} directories."
    )

    if total_archived > 0:
        answer = input(
            "Do you want to remove the original directories now that they are archived? (yes/no): "
        )
