...
DIRS_TO_REMOVE = ["test_venv", "test_venv_clean", "test_venv_temp", ".pytest_cache", "__pycache__"]

# Directories to archive (zip and then remove original)
DIRS_TO_ARCHIVE = ["benchmarks", "experiments"]
...
def archive_directory(dir_path: str) -> bool:
    ...
    archive_name = (
        f"{archives_dir}/{os.path.basename(dir_path)}_{datetime.now().strftime('%Y%m%d')}.zip"
    )
...
def remove_test_chromadb_dirs() -> int:
    ...
    for potential_dir in os.listdir("."):
        if os.path.isdir(potential_dir) and (
            potential_dir.startswith("test_chroma_")
            or potential_dir.startswith("chroma_benchmark_")
            or potential_dir.startswith("test_full_memory_pipeline_")
            or potential_dir.startswith("test_memory_utility_score_")
        ):
            try:
