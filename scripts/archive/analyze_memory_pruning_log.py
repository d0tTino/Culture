...
try:
    ...
    stats["l2_pruning_operations"] = sum(
        int(count) for count in re.findall(patterns["l2_pruning_operation"], content)
    )
    ...
    stats["l1_pruning_verifications"] = len(
        re.findall(patterns["l1_pruning_verification"], content)
    )
    stats["l2_pruning_verifications"] = len(
        re.findall(patterns["l2_pruning_verification"], content)
    )
