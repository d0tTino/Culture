...
def process_role_change(agent_state: AgentState, state_dict: dict) -> bool:
    """
    ...
    Args:
        ...
        state_dict (dict): The state dictionary for the agent.
        ...
        if requested_role != agent_state.role:
            ...
            if agent_state.steps_in_current_role >= agent_state.role_change_cooldown:
                ...
                if agent_state.ip >= agent_state.role_change_ip_cost:
                    ...
                    logger.info(
                        f"Agent {agent_state.agent_id} changed role from {old_role} to "
                        f"{requested_role}. Spent {agent_state.role_change_ip_cost} IP. "
                        f"Remaining IP: {agent_state.ip}"
                    )
                    return True
                else:
                    logger.info(
                        f"Agent {agent_state.agent_id} attempted to change to {requested_role} "
                        f"but had insufficient IP (needed {agent_state.role_change_ip_cost}, "
                        f"had {agent_state.ip})."
                    )
            else:
                logger.info(
                    f"Agent {agent_state.agent_id} attempted to change to {requested_role} "
                    f"but cooldown period not satisfied (needs {agent_state.role_change_cooldown} "
                    f"steps, current: {agent_state.steps_in_current_role})."
                )
        else:
            logger.info(
                f"Agent {agent_state.agent_id} already has role {requested_role}, "
                "no change needed."
            )
    else:
...
def test_role_change() -> None:
    ...
    state_dict = {"requested_role_change": "Innovator"}

    # Call our process_role_change function
    process_role_change(agent._state, state_dict)
    ...
    if agent._state.ip == initial_ip - role_change_ip_cost:
        print(
            f"❌ IP DEDUCTION TEST FAILED: Expected {initial_ip - role_change_ip_cost}, "
            f"got {agent._state.ip}"
        )
    else:
        print(
            f"❌ IP DEDUCTION TEST FAILED: Expected {initial_ip - role_change_ip_cost}, "
