...
# This will skip the entire module if dspy is not installed
...
@pytest.mark.ollama
def test_dspy_role_thought_generator():
    """
    Tests the direct import and execution of the DSPy role thought generator.
    """
    ...
    try:
        ...
        logger.error(f"Traceback: {traceback.format_exc()}")
        pytest.fail(f"basic_agent_graph import failed: {e}")
    ...
    try:
        ...
        logger.error(f"Traceback: {traceback.format_exc()}")
        pytest.fail(f"DSPy call failed: {e}")
