...
Usage:
    python tools/visualize_agent_memory.py --agent_id <agent_id> [--output_format text|html] [--max_length 200]
"""
...
def parse_args():
    ...
    parser.add_argument("--agent_id", required=True, help="ID of the agent to visualize memories for")
    parser.add_argument(
        "--output_format", 
        choices=["text", "html"], 
        default="text",
    ...
    parser.add_argument(
        ...
        type=int, 
        default=200, 
        help="Maximum length for displayed memory content"
    )
    parser.add_argument(
        "--chroma_dir", 
        default="./chroma_db", 
...
def get_all_l2_summaries(vector_store: ChromaVectorStoreManager, agent_id: str) -> List[Dict[str, Any]]:
...
def get_all_l1_summaries(vector_store: ChromaVectorStoreManager, agent_id: str) -> List[Dict[str, Any]]:
...
def determine_l1_l2_relationships(l1_summaries: List[Dict[str, Any]], l2_summaries: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    """
    ...
    for l2 in sorted_l2s:
        ...
        if consolidation_period:
            try:
                ...
                logger.warning(f"Invalid consolidation_period format for L2: {consolidation_period}")
...
def generate_text_visualization(
    ...
    for l2 in sorted_l2s:
        ...
        output.append(f"L2_Summary_{l2_id} (Step {step}, Period {consolidation_period}, MUS: {mus:.2f}, Retrievals: {retrieval_count}, Consolidates {len(l1s)} L1s)")
        
        # Wrap and indent the content
        wrapped_content = textwrap.fill(content, width=76, initial_indent="  ", subsequent_indent="  ")
        ...
        for l1 in sorted(l1s, key=lambda x: x.get("step", 0)):
            ...
            output.append(f"    L1_Summary_{l1_id} (Step {step}, MUS: {mus:.2f}, Retrievals: {retrieval_count}, Last: {last_retrieved})")
            
            # Wrap and indent the content
            wrapped_content = textwrap.fill(content, width=72, initial_indent="      ", subsequent_indent="      ")
            output.append(wrapped_content)
...
def generate_html_visualization(
    ...
    html.append("    .l2-summary { border: 1px solid #ddd; margin-bottom: 20px; padding: 10px; border-radius: 4px; }")
    html.append("    .l2-header { background-color: #f5f5f5; padding: 10px; margin-bottom: 10px; border-radius: 4px; }")
    html.append("    .l1-summary { border: 1px solid #eee; margin: 10px 0 10px 20px; padding: 10px; border-radius: 4px; }")
    html.append("    .l1-header { background-color: #f9f9f9; padding: 8px; margin-bottom: 8px; border-radius: 4px; }")
    html.append("    .content { white-space: pre-wrap; margin-left: 10px; }")
    html.append("    .unassigned { border-left: 5px solid #ffcccb; }")
    html.append("    .stats { background-color: #efffef; padding: 15px; margin-top: 20px; border-radius: 4px; }")
    ...
    html.append("  <p>Hierarchical structure showing L2 (chapter) summaries and their constituent L1 (session) summaries.</p>")
    ...
    for l2 in sorted_l2s:
        ...
        html.append(f"        Retrievals: {retrieval_count} | Consolidates: {len(l1s)} L1 summaries</p>")
        html.append("      </div>")
        html.append("    </div>")
        ...
        if l1s:
            ...
            for l1 in sorted(l1s, key=lambda x: x.get("step", 0)):
                ...
                html.append(f"          Retrievals: {retrieval_count} | Last Retrieved: {last_retrieved}</p>")
                html.append("        </div>")
                html.append("      </div>")
...
def main():
    ...
    if output_format == "text":
        visualization = generate_text_visualization(l2_summaries, l2_to_l1s, unassigned_l1s, max_length)
        print(visualization)
        ...
        visualization = generate_html_visualization(agent_id, l2_summaries, l2_to_l1s, unassigned_l1s, max_length)
